...
#[ 8.10623483e-05 -1.66103913e-02]

DATA_OPEN=$(echo "${DATA_RAW} " | grep ${OPENLOOPPV} | awk -v CONVFMT=%.17g -v scale=8.10623483e-05 -v offset=-1.66103913e-02] '{$NF*=scale;$NF+=offset; print $0}')
...
#[-4.45245207e-05  3.07261902e+01]
DATA_SSI=$(echo "${DATA_SSI} " | grep ${SSIPV} | awk -v CONVFMT=%.17g -v scale=-4.45245207e-05s -v offset=3.07261902e+01 '{$NF*=scale;$NF+=offset; print $0}')
